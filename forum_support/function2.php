	...
	if($arm) 
	{
		...
		if(count($temp_item2)>0)
		{
			$sql="select ifnull(sum(a.s_df),0) as s_df,ifnull(sum(a.s_mdf),0) as s_mdf,ifnull(sum(a.s_agl),0) as s_agl,ifnull(sum(a.s_at),0) as s_at,ifnull(sum(a.s_mat),0) as s_mat
				...
				from wog_stone_list a,wog_stone_setup b where b.hs_id in (".implode(",",$temp_item2).") and a.d_id in (b.hole_1,b.hole_2,b.hole_3,b.hole_4)";
...
function get_arm_sp($packs,$sum,&$temp_item,&$temp_item9)
{
	...
	if(count($temp_item2) > 0) //取出鑲嵌數值
	{
		...
		$temp_id_str=array();
		$temp_id2_str=array();
		$sql="select hs_id,hole_1,hole_2,hole_3,hole_4 from wog_stone_setup where hs_id in (".implode(",",$temp_item2).")";
		$temp_str=$DB_site->query($sql);
		while($p2=$DB_site->fetch_array($temp_str))
		{
			set_hs_id($p2[hole_1],$temp_id[$p2[hs_id]],$temp_id2[$p2[hs_id]]);
			set_hs_id($p2[hole_2],$temp_id[$p2[hs_id]],$temp_id2[$p2[hs_id]]);
			set_hs_id($p2[hole_3],$temp_id[$p2[hs_id]],$temp_id2[$p2[hs_id]]);
			set_hs_id($p2[hole_4],$temp_id[$p2[hs_id]],$temp_id2[$p2[hs_id]]);
			$temp_id_str[]=implode(",",$temp_id[$p2[hs_id]]);
			$temp_id2_str[]=implode(",",$temp_id2[$p2[hs_id]]);
		}
		$DB_site->free_result($temp_str);
				
		$temp_str=$DB_site->query("select a.d_id,a.d_name
				from wog_df a where a.d_id in (".implode(",",$temp_id_str).")") ;
		while($p2=$DB_site->fetch_array($temp_str))
		{
			//array_get_key();
			//$arm_array[hs][$p2[hs_id]][d_name].="　".$p2[d_name];
			$arm_array[hs][array_get_key($temp_id,$p2[d_id])][d_name].="　".$p2[d_name];
		}
		$DB_site->free_result($temp_str);
		...
		$sql="select b.hs_id,ifnull(sum(a.s_df),0) as s_df,ifnull(sum(a.s_mdf),0) as s_mdf,ifnull(sum(a.s_agl),0) as s_agl,ifnull(sum(a.s_at),0) as s_at,ifnull(sum(a.s_mat),0) as s_mat
			,ifnull(sum(a.s_str),0) as s_str,ifnull(sum(a.s_life),0) as s_life,ifnull(sum(a.s_vit),0) as s_vit,ifnull(sum(a.s_smart),0) as s_smart,ifnull(sum(a.s_au),0) as s_au,ifnull(sum(a.s_be),0) as s_be
			from wog_stone_list a,wog_stone_setup b where b.hs_id in (".implode(",",$temp_item2).") and a.d_id in (b.hole_1,b.hole_2,b.hole_3,b.hole_4) group by b.hs_id";
		$temp_str=$DB_site->query($sql);
		while($p2=$DB_site->fetch_array($temp_str))
		{
...
// 設定鑲嵌ID陣列
function set_hs_id($hs_id,&$temp_id,&$temp_id2)
{
	if(!empty($hs_id))
	{
		if(strlen($hs_id)==strcspn($hs_id,","))
		{
			$temp_id[]=$hs_id;
		}else
		{
...
function array_get_key($sh_array,$a1)
{
	foreach($sh_array as $key=>$val)
	{
		if(in_array($a1,$val))
		{
			return $key;
